...
$sql = "UPDATE by_order SET confirmInfo = 'True' WHERE id = '$info[by_order]'";
$status = $db->exec($sql);
if ($status) {
    $sql = "INSERT INTO log(byny, xh, lb, sj, czr, ip, client, bz) VALUES ('$info[byny]', '$info[xh]', '确认邮寄信息', '".date("Y/m/d H:i:s")."', '$info[xm]', null, 'by_web', null)";
    $status1 = $db->exec($sql);
    if ($status1) {
